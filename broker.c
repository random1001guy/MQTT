...
main (int argc, char *argv[])
{
  ...
  if (argc != 3)
    {
      printf
	("Usage : ./broker <NeighbourBroker_IP> <NeighbourBroker_Port>\n");
      exit (0);
    }
  ...
  fprintf (fptr,
	   "Broker %d started. Connected to Broker with IP : %s Port %s\n",
...
void
connect_to_neighbour (char **argv)
{
  ...
  fprintf (fptr,
	   "Broker %d : connect() to neighbouring broker successfull!\n",
	   myBrokerID);
...
let_neighbour_connect ()
{
  ...
  if ((from_neighbour_sockfd =
       accept (listenfd, (SA *) & cliaddr, &clilen)) == -1)
      ...
      for (int i = 0; i <= maxfd; i++)
	{
	  ...
	  if (FD_ISSET (i, &rset))
		...
		{		/* handle data from a client */
		  master mstr;
		  if ((bytes =
		       read (i, &mstr, sizeof (mstr))) < sizeof (mstr))
		    {
...
void
get_max_msgid_broker_file (master * mstr)
{
  FILE *fptr;
...
void
create_message_file (master mstr)
{
  FILE *fptr;
  char *file_name = (char *) malloc (strlen (mstr.topic_name) + 10 + 1);
  strcpy (file_name, mstr.topic_name);
